...
plt.tight_layout()
out_file = os.path.join(root,'figures','highres', 'number_of_studies_by_year_all_and_thematic_group_v%s.png'%v)
...
N_cols = len(rank_sum_r.columns)
M = len(rank_sum_r.index)
ylabels = rank_sum_r.columns
xlabels = rank_sum_r.index

x, y = np.meshgrid(np.arange(M), np.arange(N_cols))
...
axs[0].set(xticks=np.arange(M), yticks=np.arange(N_cols), 
       xlim=[-1, M], ylim=[-1, N_cols])
...
plt.tight_layout()
out_file = os.path.join(root,'figures','highres', 'sum_of_scores_by_year_by_thematic_group_v%s.png'%v)
...
# Save the figure
out_file = os.path.join(root,'figures','highres', 'systems_approach_quadratic_fit_v%s.png'%v)
plt.savefig(out_file, dpi=300, bbox_inches='tight')
plt.show()
...
out_file = os.path.join(root,'figures','highres', 'mann-whitney-u-test_by_thematic_group_v%s.png'%v)
plt.savefig(out_file, dpi=300, bbox_inches='tight')
plt.show()
...
# Save
out_file = os.path.join(root,'figures','highres', 'number_of_pairs_by_thematic_group_v%s.png'%v)
plt.savefig(out_file, dpi=300, bbox_inches='tight')
plt.show()   
